...
export const OrderDetail: React.FC<DetailProps> = ({ orders, role, updateStatus, addChat }) => {
  ...
  // Estados para Abonos Manuales (Solo Admin)
  ...
  // --- CÁLCULOS FINANCIEROS (Matemática de Deuda) ---
  ...
  const registerAbono = async () => {
    ...
    // Si paga todo, y estaba en crédito, pasamos a PAGADO
        ...
        <div className="lg:col-span-2 space-y-6">
             ...
             <div className="divide-y divide-slate-100">
               {order.items.map((item, idx) => (
                 <div key={idx} className="p-4 flex items-center gap-3 md:gap-4">
                   ...
                   <p className="font-black text-slate-900 text-sm w-16 text-right">${item.product.price * item.quantity}</p>
                 </div>
               ))}
             </div>
          </div>
          ...
          {(order.payments && order.payments.length > 0) || role === 'admin' ? (
             <div className="bg-white p-6 rounded-2xl shadow-lg border border-slate-100 space-y-6">
                ...
                {/* FORMULARIO ADMIN: REGISTRAR ABONO */}
          ...
          {/* --- FLUJOS DE ESTADO (COTIZACIÓN / PAGO WEB / ENVÍO) --- */}
          ...
          {order.status === OrderStatus.APPROVED && role === 'client' && (
               <div className="bg-white p-6 rounded-2xl shadow-lg border border-slate-100 space-y-4">
                 ...
                 <div className="bg-slate-50 p-4 rounded-xl border border-slate-200">
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Cuentas Panaven</p>
                    <div className="font-mono text-xs text-slate-800 space-y-1"><p>• BNC: 0191-0052-12-1234567890</p><p>• Pago Móvil: 0412-1234567 (V-12345678)</p><p>• Zelle: pagos@panaven.com</p></div>
                 </div>

                 <div className="flex bg-slate-100 p-1 rounded-xl">
                    <button onClick={() => setDeliveryMethod('shipping')} className={`flex-1 py-2 rounded-lg text-xs font-black uppercase tracking-widest transition-all ${deliveryMethod === 'shipping' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-400'}`}>🚚 Envío</button>
                    <button onClick={() => setDeliveryMethod('pickup')} className={`flex-1 py-2 rounded-lg text-xs font-black uppercase tracking-widest transition-all ${deliveryMethod === 'pickup' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-400'}`}>🏪 Retiro</button>
                 </div>

                 {deliveryMethod === 'shipping' ? (
                   <div><label className="text-[10px] font-black uppercase text-slate-500 ml-1">Dirección</label><textarea value={addressInput} onChange={e => setAddressInput(e.target.value)} placeholder="Datos exactos de envío..." className="w-full border-2 border-slate-200 p-3 rounded-xl text-sm font-bold focus:border-blue-600 outline-none h-20 resize-none" /></div>
                 ) : (
                   <div className="text-center p-4 bg-blue-50 rounded-xl border border-blue-100"><p className="text-xs font-bold text-blue-900 mb-2">📍 Te esperamos en nuestra sede</p><a href={WAZE_URL} target="_blank" className="inline-flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-lg text-xs font-black uppercase tracking-widest hover:bg-blue-700">Ver en Waze</a></div>
                 )}

                 <div className="grid grid-cols-2 gap-4">
                   <div><label className="text-[10px] font-black uppercase text-slate-500 ml-1"># Referencia</label><input value={paymentRefInput} onChange={e => setPaymentRefInput(e.target.value)} className="w-full border-2 border-slate-200 p-3 rounded-xl text-sm font-bold focus:border-blue-600 outline-none" /></div>
                   <div><label className="text-[10px] font-black uppercase text-slate-500 ml-1">Capture</label><label className={`block w-full border-2 border-dashed ${paymentRefInput ? 'border-blue-400 bg-blue-50 cursor-pointer' : 'border-slate-200 bg-slate-50 cursor-not-allowed'} p-3 rounded-xl text-center transition-all`}><input type="file" disabled={!paymentRefInput} onChange={e => handleFileUpload(e, 'paymentProof', OrderStatus.PAID)} className="hidden" /><span className="text-[10px] font-black uppercase text-blue-600">{uploading ? 'Subiendo...' : 'Subir y Pagar'}</span></label></div>
                 </div>
               </div>
          )}
          ...
          {(order.status === OrderStatus.PAID || order.status === OrderStatus.CREDIT_ACTIVE) && (
             <div className="bg-white p-6 rounded-2xl shadow-lg border border-slate-100 space-y-6">
                 {/* Si es cliente pagado web, mostramos sus datos */}
                 {order.paymentProof && (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="space-y-2"><span className="text-[9px] font-black uppercase text-slate-400">Comprobante</span><a href={order.paymentProof} target="_blank" className="block relative h-32 rounded-xl overflow-hidden border border-slate-200 group"><img src={order.paymentProof} className="w-full h-full object-cover" /><div className="absolute inset-0 bg-black/50 flex items-center justify-center opacity-0 group-hover:opacity-100 transition text-white text-xs font-bold">Ver</div></a></div>
                        <div className="space-y-2"><span className="text-[9px] font-black uppercase text-slate-400">Entrega</span>{order.shippingAddress === 'RETIRO EN TIENDA' ? (<div className="h-32 bg-blue-50 rounded-xl border border-blue-100 flex flex-col items-center justify-center text-center p-4"><span className="text-2xl">🏪</span><p className="text-xs font-black text-blue-900 uppercase mt-2">Retiro en Tienda</p></div>) : (<div className="bg-slate-50 p-3 rounded-xl border border-slate-200 h-32 overflow-y-auto text-xs font-bold text-slate-700">{order.shippingAddress}</div>)}</div>
                    </div>
                 )}

                 {role === 'admin' ? (
                   <div className="border-t pt-4 mt-4">
                     <label className="text-[10px] font-black uppercase text-slate-500 ml-1 mb-2 block">{order.shippingAddress === 'RETIRO EN TIENDA' ? 'Confirmar Entrega:' : 'Subir Guía para Enviar:'}</label>
                     <label className="flex items-center justify-center w-full py-4 bg-slate-900 text-white rounded-xl font-black uppercase text-xs tracking-widest shadow-lg hover:bg-black cursor-pointer gap-2">
                        <span>📤 {order.shippingAddress === 'RETIRO EN TIENDA' ? 'Marcar Entregado' : 'Subir Guía y Enviar'}</span>
                        <input type="file" onChange={e => handleFileUpload(e, 'shippingReceipt', OrderStatus.SHIPPED)} className="hidden" />
                     </label>
                     {uploading && <p className="text-center text-xs text-blue-600 font-bold mt-2">Procesando...</p>}
                   </div>
                 ) : (
                   order.status === OrderStatus.PAID && <p className="text-center text-xs text-slate-500 italic">Pago en verificación.</p>
                 )}
             </div>
          )}

          {/* 4. EN CAMINO / CALIFICACIÓN */}
             ...
             <div className="bg-white p-6 rounded-2xl shadow-lg border border-slate-100 text-center space-y-6">
                 ...
                 {role === 'client' ? (
                   <div className="bg-slate-50 p-6 rounded-2xl border border-slate-100 mt-6">
                     <p className="text-xs font-bold text-slate-900 uppercase mb-4">¿Ya tienes tu producto?</p>
                     <div className="flex justify-center gap-2 mb-4 text-2xl">{[1,2,3,4,5].map(star => (<button key={star} onClick={() => setRatingInput(star)} className={`transition ${star <= ratingInput ? 'text-yellow-400 scale-110' : 'text-slate-300'}`}>★</button>))}</div>
                     <textarea value={reviewInput} onChange={e => setReviewInput(e.target.value)} placeholder="Comentario..." className="w-full p-3 border border-slate-200 rounded-xl text-xs mb-4 outline-none focus:border-blue-500" />
                     <button onClick={submitRating} className="w-full py-3 bg-green-600 text-white rounded-xl font-black uppercase text-xs tracking-widest shadow-lg hover:bg-green-700">Confirmar Recepción</button>
                   </div>
                 ) : (<p className="text-xs text-slate-500">Esperando confirmación del cliente...</p>)}
             </div>
          )}
