    ...
    <div className="max-w-md mx-4 md:mx-auto mt-20 p-8 bg-white rounded-3xl shadow-2xl border border-slate-100 text-center">
      <div className="flex justify-center mb-6">
        <div className="w-24 h-24 bg-slate-50 rounded-full flex items-center justify-center border-4 border-white shadow-lg">
            <img src={LOGO_URL} className="w-20 h-20 object-contain" />
        </div>
      </div>
...
export const AdminDashboard: React.FC<{ state: AppState, updateStatus: any, addChat: any, onLogout: () => void }> = ({ state, onLogout }) => {
  ...
  // ESTADOS NUEVA VENTA (COTIZACIÓN POR PASOS)
  const [saleStep, setSaleStep] = useState(1); // 1 = Selección, 2 = Datos
  ...
  const saveProduct = async () => {
    ...
    if (validImages.length === 0) { alert("Falta foto"); return; }
    const productData = {
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="text-center md:text-left">
            ...
            <div className="flex gap-4 items-center justify-center md:justify-start mt-1">
              ...
              <span className="text-[10px] font-black text-green-600 bg-green-50 px-2 py-1 rounded-lg">Caja: ${income}</span>
            </div>
          </div>
        </div>
        
        <div className="flex flex-wrap gap-2 justify-center">
          <button onClick={() => { setShowSaleModal(true); setSaleStep(1); }} className="bg-blue-600 text-white px-6 py-3 rounded-xl font-black text-xs uppercase tracking-widest shadow-lg hover:bg-blue-700 transition flex items-center gap-2 transform active:scale-95">
          ...
          <div className="flex bg-slate-100 p-1 rounded-2xl gap-1">
            ...
            <button onClick={() => setActiveTab('debts')} className={`px-4 py-2 rounded-xl font-bold text-[10px] uppercase tracking-widest transition-all ${activeTab === 'debts' ? 'bg-orange-500 text-white shadow-md' : 'text-slate-400'}`}>Deudas ({debts.length})</button>
            ...
            {state.injectors.map(item => (
              <div key={item.id} className="bg-white p-3 rounded-2xl border border-slate-200 shadow-sm flex gap-3 relative hover:border-blue-300 transition-all group">
                ...
                <div className="flex-1 min-w-0 flex flex-col justify-between py-1">
                  ...
                  <div className="flex justify-between items-end"><span className="text-lg font-black text-slate-900">${item.price}</span><span className={`text-[9px] font-bold px-2 py-0.5 rounded ${item.stock < 5 ? 'bg-red-100 text-red-600' : 'bg-green-100 text-green-600'}`}>Stock: {item.stock}</span></div>
                </div>
           ...
           {state.orders.map(order => (
              <Link key={order.id} to={`/order/${order.id}`} className="block bg-white p-4 rounded-2xl border border-slate-100 shadow-sm hover:shadow-md transition">
                 <div className="flex justify-between items-center">
                    ...
                    <div className="text-right font-black text-lg text-slate-900">${order.items.reduce((a,b) => a + ((b.customPrice||b.product.price) * b.quantity), 0)}</div>
                 </div>
              </Link>
           ))}
        </div>
                ...
                <div className="flex-1 flex flex-col md:flex-row overflow-hidden">
                    ...
                    <div className="w-full md:w-1/2 p-6 overflow-y-auto flex flex-col bg-white">
                        ...
                        <div className="mt-auto pt-4 border-t border-slate-100">
                            <div className="flex justify-between text-xl font-black text-slate-900 mb-4"><span>Total:</span><span>${saleTotal}</span></div>
