...
export const InstallPWA: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // 2. Detectar evento de instalación (Android/Chrome/Edge)
    const handler = (e: any) => {
      e.preventDefault(); // Prevenir que el navegador muestre su propio aviso feo
      setDeferredPrompt(e);
      setShowButton(true);
    };
  ...
  return (
    <>
      {/* --- BOTÓN FLOTANTE (ANDROID / PC) --- */}
      {showButton && (
        <button 
          onClick={handleInstallClick}
          className="fixed bottom-20 right-4 z-[9999] bg-slate-900 text-white pl-4 pr-6 py-3 rounded-full shadow-2xl shadow-blue-600/50 border-2 border-white/20 font-black uppercase tracking-widest text-xs flex items-center gap-3 animate-bounce hover:bg-black transition-colors"
        >
          {/* LOGO AQUÍ */}
          <img 
            src={LOGO_URL} 
            alt="Instalar" 
            className="w-8 h-8 object-contain filter drop-shadow-[0_0_2px_rgba(255,255,255,0.3)]" 
          />
          Instalar App
        </button>
      )}
      
      {/* --- AVISO PARA IPHONE (iOS) --- */}
      {isIOS && (
        <div className="fixed bottom-0 left-0 right-0 z-[9999] bg-white/95 backdrop-blur-md p-6 rounded-t-3xl shadow-[0_-10px_40px_rgba(0,0,0,0.2)] border-t border-slate-200 text-center animate-slideUp">
           ...
           {/* LOGO AQUÍ TAMBIÉN */}
           <div className="w-14 h-14 bg-slate-100 rounded-2xl mx-auto mb-3 flex items-center justify-center shadow-sm border border-white">
              <img src={LOGO_URL} className="w-10 h-10 object-contain" alt="Logo Panaven" />
           </div>
